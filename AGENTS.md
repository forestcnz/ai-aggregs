...
- `gateway.rs` — 网关生命周期管理（启动、停止、重建、consumer models 同步）
- `state.rs` — 全局共享状态定义（`AppCtrl`、`ServerHandle`、`TrayItems`、IPC 返回类型）
- `tray.rs` — 系统托盘菜单构建和事件处理
- `error.rs` — 错误类型：`AppError`（Axum HTTP）+ `IpcError`（Tauri IPC，自动从 anyhow 转换）
