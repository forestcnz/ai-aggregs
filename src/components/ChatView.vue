...
// ---- 按协议构建请求体 ----
function buildRequestBody(model: string, sendMessages: { role: string; content: string }[]): Record<string, unknown> {
  if (protocol.value === 'responses') {
    ...
    const input = sendMessages.map((m) => {
      if (m.role === 'user') {
        ...
        return { type: 'message', role: 'assistant', content: [{ type: 'output_text', text: m.content }] }
      } else {
    ...
    return {
      model, messages: sendMessages, stream: true,
  ...
  <div class="chat-page">
    <div class="chat-header">
      <div class="chat-controls">
        ...
        <button class="btn btn-secondary sm" @click="clearChat" :disabled="sending || messages.length === 0">
          清空
        </button>
      </div>
    </div>
    ...
    <!-- 网关未运行提示 -->
    <div v-if="!status.running" class="notice">
      网关未运行，请先在「网关状态」页启动网关。
    </div>
    <!-- 无可用模型提示 -->
        ...
        <div class="msg-bubble">
          ...
          >
            <div class="thinking-header">
              ...
              <span v-if="msg.thinking" class="thinking-dots"><span></span><span></span><span></span></span>
            </div>
          ...
          <!-- 等待回复占位 -->
          <div v-if="msg.role === 'assistant' && !msg.content && !msg.reasoning && !msg.error && msg.thinking" class="msg-waiting">
